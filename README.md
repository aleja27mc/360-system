...
- Para el diseño, me ayudó a estructurar y redactar el documento de arquitectura, y también
  pedí una revisión adicional del plan que hicimos antes de pasar a la implementación, con un segundo agente
  haciendo de evaluador independiente para identificar posibles huecos de seguridad o
  inconsistencias antes de que quedaran incorporadas en el código.
- Para la implementación, me ayudó a generar gran parte del código, el esquema y pruebas,
  siguiendo el contrato que yo ya había definido en el diseño. Las decisiones de arquitectura,
  el modelo de datos y qué simplificar (y por qué) las tomé yo.
- Para las Partes 3 y 4, usé la IA para redactar de mejor forma las respuestas sobre decisiones que ya había
  tomado yo.
...
4. Si el enunciado pide el estado financiero "de inmediato", entiendo que eso implica consulta
   síncrona sin cache (o con TTL de segundos), porque ahí la exactitud pesa más que la latencia.
...
Vale la pena aclarar un punto: usar CDC como respaldo para publicar eventos es un supuesto de
bastante riesgo, porque en la práctica pocas universidades dan acceso a los redo logs de su
ERP. Lo dejo anotado como algo que habría que validar con el equipo dueño del sistema académico
antes de darlo por sentado.
...
- `403`: si quien llama no tiene permiso sobre ese `studentId`. Un `STUDENT` solo puede pedir
  su propio id; un `ADVISOR` solo puede pedir estudiantes que tenga asignados, y eso se valida
...
Es una simplificación consciente para el alcance de esta prueba.

### Seguridad de este servicio en particular
...
secreto compartido que hay aquí es únicamente para poder correr la prueba de punta a punta sin
...
### Dos errores que encontré probando el código

Los dejo anotados porque muestran cómo verifiqué esto, no solo que lo escribí:
...
servicio separados, porque para el alcance de este caso eso sería más complejidad de la que
vale la pena. Lo único que sí agregaría es un claim tipo `aud` por servicio destino, para poder
distinguir en logs y en auditoría qué servicio hizo una llamada interna, no solo qué usuario la
originó. Es útil si algún adaptador se ve comprometido o tiene un bug de logging.
...
en reposo, no solo en tránsito con TLS, porque es información de un sector regulado.
...
cache (o con un TTL de pocos segundos, como mucho). La razón es simple: el estudiante necesita ese
...
Académico. El patrón publicar/suscribir evita hacer polling y deja que cada consumidor sea
independiente de la disponibilidad del otro.
